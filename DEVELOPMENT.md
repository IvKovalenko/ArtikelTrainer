...
- **Без своего домена** Resend шлёт письма с `onboarding@resend.dev` и **только на
  ...
  пользователей. Для продакшена нужен свой домен: Resend → Domains → добавить
  DNS-записи (SPF/DKIM), затем `npx wrangler pages secret put MAIL_FROM`
  со значением вида `Artikeltrainer <noreply@твой-домен.de>`.
